...
export function evaluateTrial(evidence: TrialEvidence): TrialReport {
  ...
  for (const g of evidence.garments) {
    ...
    if (!g.storeId.trim() || !g.garmentId.trim() || ids.has(key))
      throw new Error('Missing or duplicate observation identity');
    ids.add(key);
  }
  const real = evidence.garments.filter((g) => g.source === 'real-store');
  const stores = new Set(real.map((g) => g.storeId));
  if (real.length < 50 || stores.size < 3)
    blockers.push('50 real garments from at least 3 stores are required');
  const reviewed = real.filter((g) => g.ownerWouldPublish !== null);
  const approval =
    reviewed.length === 0
      ? null
      : reviewed.filter((g) => g.ownerWouldPublish).length / reviewed.length;
  if (reviewed.length !== real.length || approval === null || approval < 0.7)
    blockers.push('All garments need owner review with at least 70% approval');
  const colors = real.flatMap((g) =>
    g.physicalLab && g.renderedLab ? [deltaE00(g.physicalLab, g.renderedLab)] : [],
  );
  const median = percentile(colors, 0.5);
  if (colors.length !== real.length || median === null || median >= 3)
    blockers.push(
      'Every garment needs a physical color measurement; median Delta E00 must be below 3',
    );
  for (const run of evidence.deviceRuns)
    if (!Number.isFinite(run.ttfdMs) || run.ttfdMs <= 0)
      throw new Error('Invalid device timing');
  const runs = evidence.deviceRuns.filter(
    (r) =>
      r.physical && r.device === 'Samsung Galaxy A16 4GB' && r.browser === 'Instagram',
  );
  ...
  if (cold.length < 10 || coldP75 === null || coldP75 >= 4000)
    blockers.push(
      '10 cold-cache reference-phone runs with p75 below 4000 ms are required',
    );
  if (warm.length < 10 || warmP75 === null || warmP75 >= 800)
    blockers.push(
      '10 warm-cache reference-phone runs with p75 below 800 ms are required',
    );
  if (evidence.typography !== 'confirmed')
    blockers.push('Arabic typography needs a recorded review');
  return {
    passed: blockers.length === 0,
    realGarments: real.length,
    realStores: stores.size,
    approvalRate: approval,
    medianDeltaE00: median,
    coldP75Ms: coldP75,
    warmP75Ms: warmP75,
    blockers,
  };
