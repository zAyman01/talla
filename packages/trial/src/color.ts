...
export function deltaE00(first: Lab, second: Lab): number {
  for (const c of [first, second]) {
    if (![c.L, c.a, c.b].every(Number.isFinite) || c.L < 0 || c.L > 100) throw new Error('Invalid CIELAB measurement');
  }
  ...
  const rt = -2 * Math.sqrt(meanC ** 7 / (meanC ** 7 + 25 ** 7)) * sin(60 * Math.exp(-(((meanH - 275) / 25) ** 2)));
