...
function candidate(id: string, edit: Partial<GarmentSpec['style']> = {}): GarmentSpec {
  return { ...structuredClone(anchor), id, style: { ...anchor.style, slot: 'bottom', ...edit } };
}
...
describe('styling rules', () => {
  it('enforces stock, tenant, selected size, slots, season and formality even for pins', () => {
    ...
    const candidates = [valid, candidate('no-stock'), candidate('season', { season: 'cold' }),
      candidate('formal', { formality: 5 }), candidate('slot', { slot: 'top' }),
      { ...candidate('tenant'), tenant_id: 'other' }];
    const result = rankSuggestions({ anchor, candidates, pins: candidates.map((s) => ({ anchorId: anchor.id, suggestedId: s.id })),
      inStock: new Set(candidates.filter((s) => s.id !== 'no-stock').map((s) => s.id)), fillSlots: ['bottom', 'top'], selectedSize: 'M' });
    expect(result.map((s) => s.garmentId)).toEqual(['valid']);
    expect(rankSuggestions({ anchor, candidates: [valid], pins: [], inStock: new Set(['valid']), fillSlots: ['bottom'], selectedSize: 'XXL' })).toEqual([]);
  });
  it('is deterministic, preserves input, deduplicates and puts eligible pins first', () => {
    const a = candidate('a');
    const b = candidate('b');
    const input = { anchor, candidates: [a, b, a], pins: [{ anchorId: anchor.id, suggestedId: 'b' }], inStock: new Set(['a', 'b']), fillSlots: ['bottom'] as const };
  ...
  it('caps margin movement at one position across many deterministic catalogs', () => {
    for (let seed = 0; seed < 100; seed++) {
      const candidates = Array.from({ length: 6 }, (_, i) => candidate(String(i), { pattern_busy: ((seed * 13 + i * 7) % 100) / 100 }));
      const input = { anchor, candidates, pins: [], inStock: new Set(candidates.map((s) => s.id)), fillSlots: ['bottom'] as const };
      const before = rankSuggestions(input);
      const after = rankSuggestions({ ...input, merchandisingPriority: new Map(candidates.map((s, i) => [s.id, (seed * 17 + i * 31) % 101])) });
      after.forEach((s, i) => {
