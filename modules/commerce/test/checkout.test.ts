...
const BUYER_PHONE = '+201000000000';
...
beforeAll(async () => {
  // The runner is the one thing that knows which migrations exist (Task 2). This file
  // still carried its own path, so it was asserting against the initial schema only.
...
function input(): CheckoutInput {
  return {
    ...
    expectedTotal: 25000,
    buyer: {
      name: conceal('اسم الاختبار'),
      phone: conceal(BUYER_PHONE),
      address: conceal('عنوان تجريبي لا يمثل مشترياً حقيقياً'),
    },
...
it('rejects a stale price and unverified phone without deducting stock', async () => {
  // Exact codes, not a substring of a message. The previous form matched on
  // `error.message`, which passes for any failure whose text happens to contain it.
  expect(await checkout.place(tenant, { ...input(), expectedTotal: 1 })).toEqual({
    ok: false,
    error: 'ORDER_TOTAL_MISMATCH',
  });
  expect(await checkout.place(tenant, { ...input(), phoneToken: 'forged' })).toEqual({
    ok: false,
    error: 'ORDER_PHONE_UNVERIFIED',
  });
...
it('places an order once, encrypts PII and replays the same receipt without another stock deduction', async () => {
  ...
  expect(await checkout.place(tenant, { ...request, expectedTotal: 1 })).toEqual({
    ok: false,
  ...
  await tenantTransaction(db, tenant, async (sql) => {
    ...
    expect(row?.buyer_ciphertext).not.toContain(BUYER_PHONE);
    if (!row) throw new Error('Missing order');
    // The column holds the plaintext, encrypted. The in-memory buyer stays concealed,
    // which is what stops it reaching a log line on the way here (ADR-0020).
    expect(privacy.open(row.buyer_ciphertext, tenant)).toEqual({
      name: 'اسم الاختبار',
      phone: BUYER_PHONE,
...
it('merges duplicate lines so they cannot oversell stock', async () => {
  const request = input();
  expect(
    await checkout.place(tenant, {
      ...request,
      lines: [...request.lines, ...request.lines, ...request.lines],
      expectedTotal: 75000,
    }),
  ).toEqual({ ok: false, error: 'STOCK_INSUFFICIENT' });
...
it('exports contact only for the matching buyer, and erases it without deleting commercial totals', async () => {
  ...
  expect(await exportBuyerOrders(database, privacy, tenant, BUYER_PHONE)).toHaveLength(1);
  expect(await applyRetention(database, tenant, 30)).toBe(0);
  expect(
    await eraseBuyerContact(database, privacy, tenant, BUYER_PHONE, 'operator-id'),
  ).toBe(1);
  expect(await exportBuyerOrders(database, privacy, tenant, BUYER_PHONE)).toEqual([]);
  await database.tenant(tenant, async (sql) => {
    const row = (
      await sql.query(
