...
const editable = new Set([
  'category', 'block_id', 'block_version', 'fabric', 'sizes_available',
  'attributes', 'attributes.sleeve_length', 'attributes.neckline', 'attributes.hem',
  'attributes.closure', 'attributes.rise', 'attributes.leg_shape',
  'style', 'style.formality', 'style.season', 'style.dominant_colors',
  'style.pattern_busy', 'style.volume', 'style.slot',
]);

function record(value: unknown): Record<string, unknown> {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) throw new Error('Invalid confirmation path');
...
export function confirmFields(spec: GarmentSpec, fields: readonly string[]): GarmentSpec {
  for (const path of fields) if (!editable.has(path)) throw new Error('Unsupported confirmation field');
  return { ...structuredClone(spec), confirmed_by_store: true,
    confirmed_fields: [...new Set([...spec.confirmed_fields, ...fields])].sort() };
}

export function preserveConfirmations(next: GarmentSpec, previous?: GarmentSpec): GarmentSpec {
  const result = structuredClone(next);
  if (!previous) return result;
  if (previous.id !== next.id || previous.tenant_id !== next.tenant_id) throw new Error('Garment identity mismatch');
